...
**确保已经被 Leader 提交的提案最终能够被所有的 Follower 提交 **很容易就能解决。因为进行投票选举的时候会比较`ZXID`，此时未接收到`commit`的`server`一定不会当选，只有`ZXID`最大的`server`，即接收到所有`commit` 的有资格成为下一个`term`的`Leader`
