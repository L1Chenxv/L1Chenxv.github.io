---
layout: post
title: 并发场景下的事务一致性如何保证？
categories: [Java]
description: 多线程情况下对事务应该如何管理
...
逛csdn时有一个问题问住了我：Spring在多线程环境下如何确保事务一致性？
四处搜索相关资料，也是找到了一篇好文

[Spring在多线程环境下如何确保事务一致性](https://cloud.tencent.com/developer/article/2138148)
...
public class MultiplyThreadTransactionManager {
    ...
    public void runAsyncButWaitUntilAllDown(List<Runnable> tasks, Executor executor) {
        ...
        // TransactionResource 保存了事务相关的上下文信息，包括资源绑定、事务名、只读状态、隔离级别和事务活动状态。通过 copyTransactionResource 方法，子线程可以复制当前的事务上下文，并通过 autoWiredTransactionResource 方法在父线程中装配这些资源，事务资源通过 TransactionResource 类来管理，最后记得在事务结束后对事物资源进行清理，以避免资源泄漏或冲突。
    }
